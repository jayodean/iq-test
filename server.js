...
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY // From Render environment variable
});
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an IQ test evaluator. Provide a short, friendly analysis based on the score and calculated IQ."
        },
        {
          role: "user",
    ...
    res.json({ iq: rawIQ, analysis: aiMessage });
  } catch (error) {
    console.error(error);
